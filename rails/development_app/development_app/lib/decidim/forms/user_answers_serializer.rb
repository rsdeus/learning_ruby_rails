    ...
    class UserAnswersSerializer < Decidim::Exporters::Serializer
      ...
      def serialize
        @answers.each_with_index.inject({}) do |serialized, (answer, idx)|
          ...
          byebug
          unless answer.body || answer.attachments.blank?
            serialized.update(
              "#{idx + 1}. #{translated_attribute(answer.question.body)}" => normalize_attachments(answer)
            )
          else
            answer.choices.map.each_with_index do |choice, idx_1|
              if choice.custom_body
                choice.custom_body.split(',').each_with_index do |choice_plus, idx_2|
                  serialized.update(
                    "#{idx + 1}.#{idx_1 + 1}.#{idx_2 + 1} #{translated_attribute(answer.question.body)}_#{choice.body}" => choice_plus.strip
                  )
                end
              else
                serialized.update(
                  "#{idx + 1}.#{idx_1 + 1} #{translated_attribute(answer.question.body)}" => choice.body
                )
              end
            end
          end
